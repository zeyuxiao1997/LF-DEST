...
class Net(nn.Module):
    ...
    def forward(self, data):
        (lf, blur, noise) = data
        # print(lf.shape, blur.shape, noise.shape)
        code = torch.cat((self.gen_code(blur), noise), dim=1)
        print('code.shape',code.shape)
        # torch.Size([4, 5, 5, 3, 18, 18]) torch.Size([4, 1, 5, 5]) torch.Size([4, 1, 5, 5])
...
class DABlock(nn.Module):
    ...
    def forward(self, x, code_array):
        ...
        fea_spa = self.relu(F.conv2d(input_spa, kernel.reshape(-1, 1, 3, 3), groups=b * u * v * c, padding=1))
