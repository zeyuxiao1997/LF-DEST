...
def p2o(psf, shape):
    ...
    # otf = torch.fft.fft(otf, 2, onesided=False)
...
def deconv_sisr_L2_fft(y, k, scale, gamma=0.01, eps=1e-2):
    ...
    STy = torch.fft.fft2(STy, dim=(-2, -1))
    STy = torch.stack((STy.real, STy.imag), -1)

    # FBFy = cmul(FBC, torch.fft.fft(STy, 2, onesided=False))
    FBFy = cmul(FBC, STy)
    ...
    # Xest = torch.fft.ifft2(FX, dim=(-2, -1)).real

    Xest = torch.fft.ifft2(torch.complex(FX[..., 0],      # 输入为数组形式
                                                FX[..., 1]), dim=(-2, -1)).real
